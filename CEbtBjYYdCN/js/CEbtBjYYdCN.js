...
function MorrisDashboard(){
	Morris.Line({
		element: 'stat-graph',
		data: [
			{"period": "2017-02", "Not Vulnerable": 13.4, "Incident reported": 55.3, 'Vulnerable #1': 1.5, 'Vulnerable #2': 0.3, 'Vulnerable #3':11, 'Vulnerable #4': 4.9, 'Vulnerable #5': 9.6 , 'Vulnerable #6':4},
		
		],
		xkey: 'period',
		ykeys: ['Win8', 'Win7','Vista','NT','XP', 'Linux', 'Mac', 'Mobile'],
		labels: ['Not Vulnerable', 'Vulnerable #1','Vulnerable #2','Vulnerable #3','Vulnerable #4', 'Incident Reported', 'Vulnerable #5', 'Vulnerable #6']
	});
	Morris.Donut({
...
function SpringyNetmap(){
	...
	var core1 = graph.newNode({label: ' Jharkhand State'});
	var core2 = graph.newNode({label: 'Ranchi District'});
	var srv1 = graph.newNode({label: 'Ranchi city'});
	var srv2 = graph.newNode({label: ' Ratu Block'});
	var pabx1 = graph.newNode({label: 'Khunti '});
	var pabx2 = graph.newNode({label: ' Ormanjhi Block'});
	var router1 = graph.newNode({label: 'kanke Block'});
	var router2 = graph.newNode({label: 'Hatia Block'});
